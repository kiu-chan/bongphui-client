            ...
            <div class="flex flex-col md:flex-row md:justify-between md:items-center mb-[46px] pt-[30px] md:pt-[50px]">
              <div>
                <h1 class="font-medium text-[32px] md:text-[45px] leading-tight">
                  Tất cả — Giải đấu đã diễn ra
                </h1>
                <p class="text-[14px] text-gray-500 mt-2">Hiển thị các giải đấu đã kết thúc. Bạn có thể chuyển trang để xem thêm.</p>
              </div>
  ...
  setup() {
    ...
    const fetchCompleted = async () => {
      ...
      try {
        ...
        console.error("Lỗi khi lấy completed:", err);
      } finally {
        loading.value = false;
      }
    };
    ...
    watch(keyword, () => { page.value = 1; fetchCompleted(); });
