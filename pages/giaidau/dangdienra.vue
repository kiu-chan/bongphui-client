            ...
            <div class="flex flex-col md:flex-row md:justify-between md:items-center mb-[46px] pt-[30px] md:pt-[50px]">
              <div>
                <h1 class="font-medium text-[32px] md:text-[45px] leading-tight">
                  Tất cả — Giải đấu đang diễn ra
                </h1>
                <p class="text-[14px] text-gray-500 mt-2">Hiển thị các giải đấu đang diễn ra. Bạn có thể chuyển trang để xem thêm.</p>
              </div>
              <div class="flex flex-wrap gap-4 mt-4 md:mt-0 items-center">
                ...
                <NuxtLink to="/giaidau">
                  <div class="w-full sm:w-[180px] h-[50px] flex justify-center items-center font-medium text-[16px] rounded-2xl buttonAdd">
                    Quay lại
                  </div>
                </NuxtLink>
              </div>
                ...
                <NuxtLink :to="`/quanlygiaidau/${item.id}/danhsachdoibong`">
                  <div class="cardAbsolute flex flex-col justify-between items-center p-[16px]">
                    <div class="max-w-[110px] h-[130px]">
                      <img class="w-full h-full object-contain" :src="item?.imageUrl" alt="" />
                    </div>
                    ...
                    <div style="color: rgba(255, 255, 255, 1);background: linear-gradient(90deg,#ec7748 0%,#a545d6 100%)" class="w-[138px] h-[40px] rounded-[5px] flex justify-center items-center font-normal text-[16px]">
                      Xem chi tiết
                    </div>
                  </div>
                </NuxtLink>
  ...
  setup() {
    ...
    const fetchOngoing = async () => {
      ...
      try {
        ...
        console.error("Lỗi khi lấy ongoing:", err);
      } finally {
        loading.value = false;
      }
    };

    onMounted(() => fetchOngoing());

    watch(keyword, () => { page.value = 1; fetchOngoing(); });

    watch(page, () => {
      if (page.value < 1) page.value = 1;
      if (suppressPageWatch.value) {
        // clear flag and skip fetch because this page change came from server sync
...
.search { border: 1px solid rgba(177, 174, 172, 1); background: rgba(255,255,255,1); }
